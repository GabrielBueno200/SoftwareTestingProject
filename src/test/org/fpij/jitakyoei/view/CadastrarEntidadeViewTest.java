...
public class CadastrarEntidadeViewTest {
    ...
    private void TestarEntidadeComCampoVazioOuInvalido(Entidade entidade, String expectedErrorMessage) {
        ...
        doNothing().when(facadeMock).createEntidade(entidade);

        try (MockedConstruction<EntidadeCadastrarPanel> panelMockScope = mockConstruction(
            EntidadeCadastrarPanel.class, 
            (entidadeCadastrarPanelMock, context) -> AddButtonsMock(entidadeCadastrarPanelMock))
        ){
            try (MockedConstruction<EntidadeForm> formMockScope = mockConstruction(
                EntidadeForm.class, 
                (entidadeFormMock, context) -> {
                    when(entidadeFormMock.getEntidade()).thenReturn(entidade);
                })
            ){
    ...
    private void AddButtonsMock(EntidadeCadastrarPanel cadastrarEntidadePanelMock)
    {
        ...
        doCallRealMethod().when(cadastrarButtonSpy).addActionListener(any(EntidadeCadastrarView.CadastrarActionHandler.class));
